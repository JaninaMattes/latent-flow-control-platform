...
class SelfAttentionTransformerBlock(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, p_dropout: float):
        super().__init__()
        ...
        self.mha = nn.MultiheadAttention(
            self.hidden_size, self.num_heads, dropout=p_dropout, batch_first=True
        )
...
class LinearBlock(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer is nn.GroupNorm and out_features % group_num != 0:
            raise ValueError(
                f"`group_num` ({group_num}) must evenly divide `out_features` ({out_features})."
            )
        ...
        self.linear = nn.Sequential(
            ...
            nn.Dropout(drop),
            norm_layer(group_num, out_features)
...
class ConvBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride, padding, act_layer
    ):
...
class TransposeConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            nn.GroupNorm(
                groups, out_channels
            ),  # Better than BatchNorm for small batch sizes
...
class InterpolateConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            ConvBlock(
                in_channels, out_channels, kernel_size, stride, padding, act_layer
            ),
            nn.GroupNorm(32, out_channels),
            act_layer()
            if callable(act_layer)
            else act_layer,  # Initialize act_layer function
        )
    ...
    def __init__(
        ...
        if scale == "same":
            ...
            self.conv2 = nn.Conv2d(
                mid_channels, mid_channels, kernel_size=3, padding=1, bias=bias
            )
...
class SelfAttentionTransformerBlock(nn.Module):
    ...
    def __init__(
        self, hidden_size: int, num_heads: int, p_dropout: float, mlp_ratio=4.0
    ):
        super().__init__()
...
class DiTAttentionBlock(nn.Module):
    def __init__(
        self, hidden_size, num_heads, mlp_ratio=4.0, p_dropout=0.0, **block_kwargs
    ):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        ) ** 2  # Number of patches (L) or tokens
        self.patch_dim = self.in_channels * (
            self.patch_size**2
        ...
        self.transformer_blocks = nn.Sequential(
            *[
                SelfAttentionTransformerBlock(
                    self.hidden_size, self.num_heads, self.p_dropout
                )
    ...
    def patchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.permute(
            0, 2, 4, 1, 3, 5
        ).contiguous()  # Size: (B, H, W, C, patch_size, patch_size)
        x = x.view(
