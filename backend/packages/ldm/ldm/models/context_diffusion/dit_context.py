...
class ContextEmbedderWithCFG(nn.Module):
    ...
    def __init__(self, context_embedding_size, hidden_size, dropout_prob):
        ...
        )  # learned null-token for dropped context vectors
    ...
    def token_drop(self, context, drop_prob):
        ...
        """
        drop_mask = (
            torch.rand(context.size(0), device=context.device) < drop_prob
        )  # (N,)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        if cat_context:
            if context_dropout_prob > 0:
                self.context_embedder = ContextEmbedderWithCFG(
                    context_size, hidden_size, context_dropout_prob
                )
    ...
    def initialize_weights(self):
        ...
        cls_token, extra_tokens = (
            bool(self.y_embedder),
            self.extras,
        )  # 1 if class-conditional, 0 otherwise
    ...
    def forward(self, x, t, y=None, context=None):
        ...
        if self.y_embedder is not None:
            ...
            if y is None:
                y = torch.full(
                    (x.size(0),), 1000, dtype=torch.long, device=x.device
                )  # 1000 labels in ImageNet
        ...
        if self.context_embedder is not None and context is not None:
            context_emb = self.context_embedder(context)  # (N, D)
            context_emb = self.context_embedder(context).unsqueeze(1)  # (N, 1, D)
            x = torch.cat(
                [x, context_emb], dim=1
            )  # (N, T+1, D) append context to img tokens
...
if __name__ == "__main__":
    ...
    print("Conditional")
    print(
        f"{'Params':<10}: {sum([p.numel() for p in net.parameters() if p.requires_grad]):,}"
    )
