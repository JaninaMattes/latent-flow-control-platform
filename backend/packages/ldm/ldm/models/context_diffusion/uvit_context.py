...
class ContextUViT(nn.Module):
    def __init__(
        ...
        self.num_features = self.hidden_size = (
            hidden_size  # num_features for consistency with other models
        )
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_channels=in_channels, hidden_size=hidden_size
        )
        ...
        if cat_context:
            ...
            print(f"[UViT] Context-conditional ({context_size} dim)")
        else:
            self.context_embedder = None
            print("[UViT] Unconditional")
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.extras + num_patches, hidden_size)
        )
    ...
    def forward(self, x, t, context=None, y=None):
        ...
        """
        return self.compiled_forward(x, t, context, y)

    def _forward(self, x, t, context=None, y=None):
        ...
        if self.context_embedder is not None and context is not None:
            context_emb = self.context_embedder(context).unsqueeze(
                1
            )  # (N, D) -> (N, 1, D)
            x = torch.cat(
                [x, context_emb], dim=1
