...
class ConcatSkipLayer(nn.Module):
    ...
    def forward(self, x, skip_input):
        if x.shape[-1] != skip_input.shape[-1]:
            raise ValueError(
                f"Shape mismatch: x ({x.shape[-1]}) and skip_input ({skip_input.shape[-1]}) must have the same feature dimension."
            )
...
class HiloSkipLayer(nn.Module):
    ...
    def __init__(
        self, embed_dim, num_heads, mlp_ratio=4.0, qkv_bias=True, net_type="decoder"
    ):
        super().__init__()
...
class Encoder_AE_MLP(BaseEncoder):
    ...
    def forward(self, x, output_layer_levels: List[int] = None):
        ...
        if self.use_checkpoint:
            x, skips = torch.utils.checkpoint.checkpoint(
                self._forward, x, output_layer_levels
            )
        else:
            x, skips = self._forward(x, output_layer_levels)
        return AEEncoderOutput(z_enc=x, skips=skips)
    ...
    def forward(self, x, skips=None, output_layer_levels: List[int] = None):
        if self.use_checkpoint:
            x = torch.utils.checkpoint.checkpoint(
                self._forward, x, skips, output_layer_levels
            )
        else:
