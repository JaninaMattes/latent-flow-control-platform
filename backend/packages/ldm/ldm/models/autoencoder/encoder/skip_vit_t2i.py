...
class FinalLayer(nn.Module):
    ...
    def __init__(self, latent_dim, embed_dim, moments_factor=2.0):
        super().__init__()
...
""" Skip Vision Transformer Encoder (Optional: Bottleneck Layer) """
...
class SkipViTEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        moments_factor=2.0,  # for VAE since: mu and logvar
        use_bottleneck_layer=True,
        compile=True,
    ):
        ...
        self.patch_embed = PatchEmbed(
            image_size, patch_size, in_channels, embed_dim, bias=True
        )
    ...
    def forward(self, x):
        ...
        # (Optional) VAE Bottleneck layer
        if self.final_layer is not None:
            z_enc = self.final_layer(x)  # Output: z_enc [mu, logvar]
        else:
            z_enc = x  # CLS token

        return AEEncoderOutput(z_enc=z_enc, skips=None)
