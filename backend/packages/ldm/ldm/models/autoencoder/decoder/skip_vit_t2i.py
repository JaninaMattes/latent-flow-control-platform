...
class CrossAttnSkipLayer(nn.Module):
    ...
    def __init__(
        self, embed_dim, dropout=0.0, proj_drop=0.0, num_heads=2, batch_first=True
    ):
        super().__init__()
        self.norm1 = nn.LayerNorm(embed_dim, eps=1e-6)
        self.norm2 = nn.LayerNorm(embed_dim, eps=1e-6)
        self.attn = nn.MultiheadAttention(
            embed_dim, num_heads=num_heads, dropout=proj_drop, batch_first=batch_first
        )
    ...
    def forward(self, x, skip_input):
        ...
        attn_output, _ = self.attn(
            self.norm1(x), self.norm2(skip_input), self.norm2(skip_input)
        )
...
class SelfAttnSkipLayer(nn.Module):
    ...
    def forward(self, x, skip_input):
        # Perform attention and add dropout for regularization
        attn_output, _ = self.attn(
            self.norm1(skip_input), self.norm1(skip_input), self.norm1(skip_input)
        )
        return x + self.dropout(attn_output)
...
class FinalLayer(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            act_layer()
            if use_act_layer
            else nn.Identity(),  # For pixel-space outputs in range [-1, 1]
        )
...
class RefineMentLayer(nn.Module):
    """
    An optional output refinement layer.
    """

    def __init__(self, in_channels, out_channels=None, act_layer: nn.Module = nn.GELU):
        super().__init__()
        out_channels = out_channels or in_channels

        self.refconv = nn.Sequential(
...
class SkipViTDecoder(BaseDecoder):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.randn(1, self.num_patches + self.extra_tokens, embed_dim)
            if pos_embed_learned
            else torch.zeros(1, self.num_patches + self.extra_tokens, embed_dim),
            requires_grad=pos_embed_learned,
        )
        ...
        self.final_layer = FinalLayer(
            embed_dim, patch_size, in_channels, use_act_layer=use_act_layer
        )
        ...
        if self.use_weighted_fft:
            self.beta_weight = nn.Parameter(
                torch.zeros(1, embed_dim), requires_grad=True
            )
            self.sigmoid = nn.Sigmoid()

        if refine_output:
            self.refine_layer = RefineMentLayer(in_channels, act_layer=act_layer)
        else:
    ...
    def forward(self, x):
        ...
        mask_tokens = self.masked_tokens.expand(
            B, -1, -1
        ...
        if self.refine_layer is not None:
            x = self.refine_layer(x)  # (B, C, H, W)

        # Return AEDecoderOutput
    ...
    def add_fft_features(self, x, skips):
        ...
        if self.use_weighted_fft:
            ...
            fft_features = self.sigmoid(self.beta_weight) * self.norm_layer(
                fft_features
            )
    ...
    def initialize_weights(self):
        ...
        if self.pos_embed_learned:
            ...
            pose_embed = get_2d_sincos_pos_embed(
        ...
        torch.nn.init.normal_(self.masked_tokens, std=0.02)

        # Initialize skip layers weights
        if self.use_weighted_fft:
            torch.nn.init.normal_(self.beta_weight, std=0.02)

        # Zero-out output layers
        nn.init.constant_(self.final_layer.mlp[0].weight, 0)
        nn.init.constant_(self.final_layer.mlp[0].bias, 0)

        # Initialize conv layers in the refinement layer
        if self.refine_layer is not None:
            for m in self.refine_layer.refconv.modules():
