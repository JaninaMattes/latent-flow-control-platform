...
def load_partial_state_dict(model: nn.Module, state_dict: dict):
    ...
    if isinstance(state_dict, dict) and "state_dict" in state_dict:
        print(
            "[load_partial_state_dict] Unwrapping nested 'state_dict' from checkpoint."
        )
    ...
    if "pos_embed" in state_dict and "pos_embed" in model.state_dict():
        if state_dict["pos_embed"].shape != model.state_dict()["pos_embed"].shape:
            print(
                f"[load_partial_state_dict] Skipping 'pos_embed' due to shape mismatch."
            )
    ...
    if ignored_keys:
        print(
            f"[load_partial_state_dict] Ignored {len(ignored_keys)} unmatched keys. Example: {ignored_keys[:5]}"
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if ckpt_path is not None:
            ...
            print(
                "[DiTWrapper] Weights loaded. You may call `.eval()` manually if needed."
            )
