    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model_type not in SiT_models:
            raise ValueError(
                f"Model type {model_type} not found in SiT_models. Available options are: {list(SiT_models.keys())}"
            )
        ...
        if ckpt_path:
            ...
            if "pos_embed" in state_dict:
                ...
                if ckpt_pos_embed.shape != pos_embed_shape:
                    print(
                        f"Resizing pos_embed from {ckpt_pos_embed.shape} to {pos_embed_shape}."
                    )
                    state_dict["pos_embed"] = self._resize_pos_embed(
                        ckpt_pos_embed, pos_embed_shape
                    )
