...
def latent2rgb(
    latents: torch.Tensor, projection: torch.Tensor | None = None
) -> torch.Tensor:
    """
    ...
    rgb = torch.clamp(
        (rgb - rgb.min()) / (rgb.max() - rgb.min()), 0, 1
    )  # normalize to [0, 1]
    return rgb


def vis_taerget_pred_grid(
    target_img: torch.Tensor,
    pred_img: torch.Tensor,
    add_img: torch.Tensor | None = None,
...
class TrainerModuleLatentFlow(LightningModule):
    def __init__(
        ...
        second_stage_cfg: Optional[
            dict
        ] = None,  # Second stage  (LDM using Flow Matching)
        third_stage_cfg: Optional[
            dict
        ] = None,  # Third stage   (Autoencoder pre-trained on LDM latent noised codes)
    ...
    def encode_second_stage(
        ...
        if exists(self.second_stage):
            xt, _ = self.second_stage.encode(
                latent, search_key=t, y=y, **sample_kwargs
    ...
    def get_data_pair(
        ...
        if not exists(x1_latent):
            x1_latent = self.encode_first_stage(img).to(
                torch.float32
            )  # latent (x1): VAE encoded img
        if not exists(xt_latent):
            xt_latent = self.encode_second_stage(
                x1_latent, t=self.source_timestep, y=label, **sample_kwargs
            ).to(torch.float32)  # latent (xt): LDM noise code

        # Extract z-vector context
        if use_context:
            context = self.encode_third_stage(xt_latent).to(
                torch.float32
            )  # Vector z context: extracted semantic context vector based on xt
    ...
    def forward(self, img, x1_latent=None, xt_latent=None, label=None, **kwargs):
        ...
        loss = self.model.training_losses(
            x1=x1_latent, context=context, y=label, **kwargs
        )
        return loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        xt_latent = default(batch[f"latents_{self.source_timestep:.2f}"], None).to(
            torch.float32
        )  # Conditioning latent (xt): LDM noise code
        x1_latent = default(batch[f"latents_{self.target_timestep:.2f}"], None).to(
            torch.float32
        )  # X1 (data): VAE encoded img
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if hasattr(self.model, "validation_losses"):
            _, val_loss_per_segment = self.model.validation_losses(
                x1_latent, context=context, y=label
            )
        ...
        pred_samples = sampler.generate(
            x=z, y=label, context=context, **self.sample_kwargs
        )
        ...
        if batch_idx % self.log_every == 0:
            ...
            if self.vis_samples is None:
                ...
                # append
                self.vis_samples["img_gt"] = torch.cat(
                    [self.vis_samples["img_gt"], real_img], dim=0
                )
                self.vis_samples["img_pred"] = torch.cat(
                    [self.vis_samples["img_pred"], fake_img], dim=0
                )
                self.vis_samples["latent_gt"] = torch.cat(
                    [self.vis_samples["latent_gt"], latent_gt], dim=0
                )
                self.vis_samples["latent_target"] = torch.cat(
                    [self.vis_samples["latent_target"], latent_target], dim=0
                )
                self.vis_samples["latent_pred"] = torch.cat(
                    [self.vis_samples["latent_pred"], latent_pred], dim=0
                )
    ...
    def on_validation_epoch_end(self):
        ...
        z = torch.randn((self.batch_size, *self.latent_shape), generator=g).to(
            self.device
        )

        # ignored in model for unconditional training
        if self.num_classes > 0:
            y = torch.randint(0, self.num_classes, (self.batch_size,), generator=g).to(
                self.device
            )
        else:
            y = None
        ...
        if self.vis_samples is not None:
            out_imgs = vis_taerget_pred_grid(
                self.vis_samples["img_gt"], self.vis_samples["img_pred"]
            )
            out_latents = vis_taerget_pred_grid(
                self.vis_samples["latent_gt"],
                self.vis_samples["latent_target"],
                self.vis_samples["latent_pred"],
            )
