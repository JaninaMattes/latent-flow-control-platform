...
from ldm.dataloader.hdf5_dataset.dataloader import HDF5DataModule
...
class HDF5DataHandler:
    def __init__(self, file_path: str, generate: bool = False) -> None:
    ...
    def save_intermediates(self, group: h5py.Group, data: Dict[str, Any]) -> None:
        if "intermediates" and "intermediate_steps" in data:
            ...
            for enc_img, t_step in zip(intermediates, intermediate_steps):
                t_step_float = (
                    t_step.item() if isinstance(t_step, torch.Tensor) else t_step
                )
    ...
    def create_or_append(
        self, group: h5py.Group, key: str, new_data: np.ndarray, dtype: np.dtype
    ) -> None:
        if key not in group:
    ...
    def save_to_numpy(self, data: Dict[str, Any], group_name: str = "train") -> str:
        ...
        if "image" in data:
            ...
            for i, image in enumerate(images):
                ...
                plt.imsave(
                    image_filename, image.transpose(1, 2, 0)
                )  # Transpose to (H, W, C)
    ...
    def save_to_hdf5(
        self, data: Dict[str, Any], group_name: str = "train", filename="data.hdf5"
    ) -> str:
    ...
    def load_data_by_keys(
        self, keys: List[str], group_name: str = "train"
    ...
    def load_from_hdf5(
        self, base_dir: Path, batch_size: int = 16, timestep: int = 0.0
    ) -> wds.WebLoader:
        """
        Load data using WebLoader from the specified numpy directory structure.
        """
        loader = HDF5DataModule(
            base_dir=base_dir, batch_size=batch_size, timestep=timestep
        )
