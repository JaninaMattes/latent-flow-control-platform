...
class Annealer:
    ...
    def get_weight_at(self, step):
        ...
        if self.total_steps > 0:
            ...
            progress = (
                step_for_calc / self.total_steps
...
class BetaAnnealer:
    ...
    def __init__(
        ...
        cycle_mode="standard",
        num_cycles=1,  # Overall cycle control
        # Standard mode specific
        total_steps=10000,
        mirror=False,  # Use total_steps as segment_duration for standard
        # Fu et al. mode specific
        cycle_length=None,
        ramp_ratio=0.5,  # Use cycle_length and ramp_ratio for Fu et al.
    ):
        """
        Parameters:
        ...
        self.cooldown_value = (
            float(cooldown_value) if cooldown_value is not None else self.warmup_value
        )
        ...
        if self.cycle_mode == "standard":
            ...
            self.segment_duration = int(
                self.cycle_length * self.ramp_ratio
        ...
        self.annealer = Annealer(
            total_steps=ramp_duration_for_annealer,
            shape=self.shape,
    ...
    def __call__(self):
        ...
        if self.current_step < self.warmup_end:
            ...
            step_in_annealing = (
                self.current_step - self.warmup_end
            )  # Step count within the total annealing duration
            step_in_cycle = (
                step_in_annealing % self.cycle_length
            ...
            if self.cycle_mode == "standard":
                ...
                segment_index_in_cycle = (
                    step_in_cycle // self.segment_duration
                ...
                if self.mirror and segment_index_in_cycle == 1:
                    ...
                    annealer_step_for_segment = (
                        self.segment_duration - 1 - step_in_segment
                    )  # Maps 0..N-1 -> N-1..0
                else:
                ...
                if self.reverse_segment and 1.0 - self.baseline > 1e-6:
                    progress_in_range = (base_weight - self.baseline) / (
                        1.0 - self.baseline
                    )
                    reversed_progress = 1.0 - progress_in_range
                    beta_weight = (
                        self.baseline + (1.0 - self.baseline) * reversed_progress
                    )
                else:
    ...
    def get_ramp_duration(self):
        ...
        return (
            self.segment_duration
        )  # segment_duration is the duration of the ramp part for both modes
...
if __name__ == "__main__":
    ...
    for i in range(12):
        print(
            f"Step {i}: KLD before: {kld_value.item():.4f}, Annealed KLD: {annealer(kld_value).item():.4f}, Weight: {annealer.get_weight():.4f}"
        )
        annealer.step()
    ...
    beta_annealer = BetaAnnealer(
        ...
        cooldown_value=0.5,
        baseline=0.0,
    )  # Baseline doesn't affect beta if max_beta is used directly?
    ...
    new_beta_annealer = BetaAnnealer(
        total_steps=10,
        max_beta=4.0,
        shape="linear",
        warmup_steps=5,
