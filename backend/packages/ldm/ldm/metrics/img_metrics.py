...
from torchmetrics.image import PeakSignalNoiseRatio as PSNR  # noqa: N817
from torchmetrics.image import StructuralSimilarityIndexMeasure as SSIM  # noqa: N817

# helper
from torchmetrics.image.fid import FrechetInceptionDistance
from torchmetrics.image.lpip import (
    LearnedPerceptualImagePatchSimilarity as LPIPS,  # noqa: N817
)
...
class ImageMetricsTracker(nn.Module):
    ...
    def __call__(self, target, pred, noise_target=None, noise_pred=None):
        ...
        self.ssims.append(
            self.ssim(pred, target)
        )  # SSIM expects [0, 1] range, or None for self-determined range
        self.psnrs.append(
            self.psnr(pred, target)
        )  # PSNR expects [0, 1] range, or None for self-determined range
        self.mses.append(
            torch.mean((pred - target) ** 2, dim=[1, 2, 3])
        )  # MSE  expects [0, 1] range
        self.maes.append(
            torch.mean(torch.abs(pred - target), dim=[1, 2, 3])
        )  # MAE  expects [0, 1] range

        # LPIPS range [0, 1] -> [-1, 1]
        self.lpips_scores.append(
            self.lpips(pred * 2 - 1, target * 2 - 1)
