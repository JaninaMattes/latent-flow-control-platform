...
class NoiseMetricsTracker(nn.Module):
    ...
    def __call__(self, img_target, img_pred, noise_target=None, noise_pred=None):
        ...
        if noise_target is not None and noise_pred is not None:
            target = denorm_tensor(
                noise_target, min=0, max=1, keep_channels=3
            ).float()  # 3 channels
            pred = denorm_tensor(
                noise_pred, min=0, max=1, keep_channels=3
            ).float()  # 3 channels

            # Compute metrics over the whole image (3-channels)
            self.cosine_sims.append(
                calculate_CosineSim(pred, target)
            )  # Cosine Similarity
