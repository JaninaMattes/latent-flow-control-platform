...
class WebDataModuleFromConfig(pl.LightningDataModule):
    ...
    def make_loader(self, dataset_config, train=True):
        ...
        transform_dict = {
            dkey: load_partial_from_config(transforms_config[dkey])
        ...
        if "dataset_transforms" in dataset_config:
            dataset_transforms = instantiate_from_config(
                dataset_config["dataset_transforms"]
            )
        else:
        ...
        if isinstance(dataset_config.shards, str):
        ...
        elif isinstance(dataset_config.shards, list) or isinstance(
            dataset_config.shards, ListConfig
        ):
            ...
            for shard in dataset_config.shards:
                ...
                if "{" in shard:
                    ...
                    tars.extend(
                        [
                            shard.replace(f"{{{start:06d}..{end:06d}}}", f"{i:06d}")
    ...
    def __iter__(self):
        for idx in range(self.start, self.end):
            with h5py.File(self.base_dir, "r", libver="latest", swmr=True) as h5_file:
                image = torch.tensor(
                    h5_file[self.group_name]["images"][idx], dtype=torch.float32
                )
                label = torch.tensor(
                    h5_file[self.group_name]["labels"][idx], dtype=torch.long
                )
                if self.timestep <= 1.0:
                    latent = torch.tensor(
                        h5_file[self.group_name]["latents"][idx], dtype=torch.float32
                    )
                else:
                    latent = image
...
class HDF5MultiLatentsDataset(Dataset):
    def __init__(
        ...
        with h5py.File(self.base_dir, "r") as h5_file:
            self.dataset_size = h5_file[group_name]["images"].shape[
                0
            ]  # Get total no samples
    ...
    def __getitem__(self, idx):
        ...
        if not self.lazy_loading:
            ...
            # --- Lazy loading ---
            self.dataset = (
                h5py.File(self.base_dir, "r")[self.group_name]
...
class HDF5MultiDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.base_dir = (
            ...
            else self.resolve_base_dir(base_dir)
        )
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        if self.train_dataset is None:
            raise ValueError(
                "Train dataset is not initialized. Did you call `setup(stage='fit')`?"
            )
    ...
    def val_dataloader(self) -> DataLoader:
        ...
        if self.val_dataset is None:
            raise ValueError(
                "Validation dataset is not initialized. Did you call `setup(stage='fit')`?"
            )
        use_multiprocessing = self.num_workers > 0
        val_data = DataLoader(
            self.val_dataset,
    ...
    def test_dataloader(self) -> DataLoader:
        ...
        if self.test_dataset is None:
            raise ValueError(
                "Test dataset is not initialized. Did you call `setup(stage='test')`?"
            )
        use_multiprocessing = self.num_workers > 0
...
class NumpyLatentsDataset(Dataset):
    def __init__(
        ...
        if self.timestep <= 1.0:
            self.latent_dir = (
                Path(self.base_dir) / group_name / f"Latents_{self.timestep:.2f}"
            )
        else:
...
class WebDataModuleFromNumpyConfig(pl.LightningDataModule):
    def __init__(
        ...
        super().__init__()
        self.base_dir = (
            base_dir if isinstance(base_dir, str) else self.resolve_base_dir(base_dir)
        )
        print(
            f"[WebDataModuleFromNumpyConfig] Setting base directory to {self.base_dir}"
        )
...
class WebDataModuleFromNumpy(pl.LightningDataModule):
    ...
    def make_loader(
        self, group_name, timestep: float = 0.0, train: bool = True
    ) -> DataLoader:
        ...
        return DataLoader(
            ...
            else None,  # Overlap data loading with model training
            drop_last=self.drop_last,
        )
...
class ImageNet256MultiDummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample[f"latents_{self.source_timestep:.2f}"] = torch.randn(
            4, 32, 32, dtype=torch.float32
        )
        sample[f"latents_{self.target_timestep:.2f}"] = torch.randn(
            4, 32, 32, dtype=torch.float32
        )
        ...
        for key, shape in self.keys_shapes.items():
            sample[key] = (
                torch.randn(*(int(dim) for dim in shape))
                if len(shape) > 1
