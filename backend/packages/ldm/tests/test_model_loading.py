...
@pytest.mark.slow
def test_beta_vae_model_loads_successfully() -> None:
    """
...
@pytest.mark.slow
def test_flow_matching_model_loads_successfully() -> None:
    """
    Integration test: load real checkpoint and verify model setup.
    """
    checkpoint = (
    ...
    fm_module = TrainerModuleLatentFlow.load_from_checkpoint(
        checkpoint_path=str(checkpoint), map_location="cpu", weights_only=False
    )
