...
def exists(val: Any) -> bool:  # noqa: ANN401
    return val is not None


def default(val: Any, d: FunctionType) -> FunctionType:  # noqa: ANN401
    if exists(val):
        return val
...
def seed_everything(seed: int) -> None:
    """Set the seed to generate random numbers on all devices"""
...
def get_obj_from_str(string: str, reload: bool = False) -> Any:  # noqa: ANN401
...
def load_model_from_config(config, ckpt, verbose=False, ignore_keys=[]) -> nn.Module:
...
def load_model_from_ckpt(
    model: nn.Module, ckpt: str, verbose: bool = False, ignore_keys: list = []
) -> nn.Module:
    print(f"Loading model from {ckpt}")
    pl_sd = torch.load(ckpt, map_location="cpu")
    if "global_step" in pl_sd:
...
def get_batch_stats(x: torch.Tensor) -> dict:
...
def resize_ims(
    x: torch.Tensor, size: int, mode: str = "bilinear", **kwargs: Any
) -> torch.Tensor:
...
def plot_hist(
    predicted: torch.Tensor, original: torch.Tensor | None = None, bins=100
) -> Figure:
    # matplotlib
    fig = plt.figure(figsize=(10, 7.5))
    if original is not None:
        plt.hist(
            original.detach().cpu().numpy().flatten(),
            bins=bins,
            alpha=0.5,
            label="original",
            density=True,
...
def ims_to_grid(ims: torch.Tensor, stack: str = "row", split: int = 4) -> torch.Tensor:
...
def un_normalize_ims(ims: torch.Tensor) -> torch.Tensor:
...
def denorm_tensor(
    tensor: torch.Tensor, min: int = 0, max: int = 255, keep_channels: int = 3
) -> torch.Tensor:
